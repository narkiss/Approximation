...
//непосредственно сама графическая часть
...
public:
	...
	//функция рисующая граффик введённой функции
		...
		vector<double>& x_first_graph = vector<double>(), vector<double>& x_second_graph = vector<double>(), vector<double>& x_ellipse = vector<double>())
	{
		...
		double data_y_max(4), data_y_min(0);
		double data_x_max(3), data_x_min(0);
		...
		{
			memDC.Ellipse(x3[i] - 3, y3[i] - 3, x3[i] + 6, y3[i] + 6);
		}
		dc->BitBlt(0, 0, frame.Width(), frame.Height(), &memDC, 0, 0, SRCCOPY);
	}
